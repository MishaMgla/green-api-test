...
export function mapNotification(body: unknown): MappedNotification {
  ...
  const timestamp = Number.isFinite(seconds) ? (seconds as number) * 1000 : Date.now()
  return { type: 'message', chatId, chatName, message: { id, text, outgoing, timestamp } }
}
 ...
 * same array when the message is already known. Shared by send and receive.
 */
export function insertMessage(
  ...
  if (chat.messages.some((existing) => existing.id === message.id)) return chats
